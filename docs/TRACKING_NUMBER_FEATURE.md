...
- Modified CTA button logic:
  - When status is "shipped" and tracking number exists: Link to Shiplifier tracking page
  - Otherwise: Link to confirmation page
- Tracking URL format: `https://tracking.skynetexpress.com/go/plm-tracking?stid=skynet&cn=<TRACKING-NUMBER>`
...
- Button opens Shiplifier tracking page in new tab with the tracking number
...
2. Email contains "Track Your Order" button linking to Shiplifier tracking page
...
4. On confirmation page, customer sees:
   ...
   - **"Track Your Parcel" button** (purple, links to Shiplifier tracking)
...
```
https://tracking.skynetexpress.com/go/plm-tracking?stid=skynet&cn=<TRACKING-NUMBER>
```
...
- [ ] Tracking link opens Shiplifier page with correct ID number
