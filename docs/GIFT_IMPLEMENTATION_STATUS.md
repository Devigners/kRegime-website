    ...
    <div className="mt-8 p-6 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-lg">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <input
            value={`${process.env.NEXT_PUBLIC_APP_URL}/gift/${order.gift_token}`}
            readOnly
...
const copyGiftLink = () => {
  navigator.clipboard.writeText(
    `${process.env.NEXT_PUBLIC_APP_URL}/gift/${order.gift_token}`
  );
  // Show success toast
};
  ...
  order.is_gift && (
    <div className="mt-2 space-y-1">
      ...
      <div className="text-xs text-gray-600">
        <div>
          <strong>From:</strong> {order.gift_giver_name}
        </div>
        <div>
          <strong>Email:</strong> {order.gift_giver_email}
        </div>
      </div>
...
```typescript
const [filterGiftStatus, setFilterGiftStatus] = useState<
  'all' | 'gifts' | 'claimed' | 'unclaimed'
>('all');
