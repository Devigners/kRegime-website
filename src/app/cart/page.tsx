...
function CartContent() {
  ...
  const [appliedDiscountCode, setAppliedDiscountCode] =
    useState<DiscountCodeInfo | null>(null);
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      if (isGift && productId) {
        setIsGiftOrder(true);
        loadProductForGift(productId, subscription);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]);

  const loadProductForGift = async (
    productId: string,
    subscription: SubscriptionType
  ) => {
    try {
      // Fetch regime data
      const response = await fetch(`/api/regimes/${productId}`);
      if (!response.ok) throw new Error('Failed to fetch regime');

      const result = await response.json();
      const regime: Regime = result.data || result; // Handle both wrapped and unwrapped responses

      const priceInfo = calculatePrice(regime, subscription);

      const giftCartData: CartData = {
        regimeId: productId,
        regime,
      ...
      setCartData(giftCartData);

      // Save to localStorage using the standard cart key for gift flow
  ...
  const handleApplyDiscountCode = async () => {
    ...
    if (
      cartData &&
      hasRegimeDiscount(cartData.regime, cartData.subscriptionType)
    ) {
      const discountPercentage = currentPriceInfo?.discount || 0;
      setDiscountCodeError(
        `Discount codes cannot be combined with regime discounts. You're already getting ${discountPercentage}% off!`
      );
      return;
    }
      ...
      if (result.success) {
        ...
        if (cartData) {
          ...
          const updatedCartData = {
            ...
            totalAmount: priceInfo.discountedPrice,
            finalAmount:
              priceWithCode.finalPriceWithCode || priceInfo.discountedPrice,
          };

          setCartData(updatedCartData);

          // Use standard cart storage for both gift and regular orders
  ...
  const currentPriceInfo = cartData
    ? calculatePrice(cartData.regime, cartData.subscriptionType)
    : null;
  ...
  const handleEditAnswers = () => {
    ...
    router.push(
      `/regime-form?product=${cartData.regimeId}&subscription=${cartData.subscriptionType}`
    );
  };
    ...
    <div className="container section-padding py-40">
      {isGiftOrder ? (
        <div className="flex flex-col gap-3">
          <h1 className="text-3xl font-bold text-black">Gift a Regime</h1>
          <p className="text-black mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Form Answers Section - Only for non-gift orders */}
          {!isGiftOrder && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
                ...
                >
                  {Object.entries(cartData.formData).map(([key, value]) => {
                    ...
                    return (
                      <div
                        key={key}
                        className="border-b border-gray-100 pb-3 last:border-b-0 group"
                      >
                ...
                <div className=" border-gray-300 rounded-lg">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyPress={(e) =>
                        e.key === 'Enter' && handleApplyDiscountCode()
                      }
                ...
                <div className="border-2 border-primary rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Tag size={16} />
                      <span className="text-sm font-bold">
                        Code {appliedDiscountCode.code} Applied
                      </span>
                    </div>
                    <button
              ...
              {appliedDiscountCode && (
                <div className="flex justify-between ">
                  <span className="font-semibold">
                    Discount ({appliedDiscountCode.percentageOff}%)
                  </span>
                  <span className="font-semibold flex items-center gap-1">
                    ...
                    {(() => {
                      ...
                      const priceWithCode = applyDiscountCode(
                        priceInfo,
                        appliedDiscountCode
                      );
